...
* The metadata for an adapter is synchronized to Rhodes along with the rest of the adapter dataset.  Metadata is called before the query method in the [`SyncEngine` workflow](../guide/using_rhoconnect#sync-workflow), so you can tailor your query method based on metadata.
