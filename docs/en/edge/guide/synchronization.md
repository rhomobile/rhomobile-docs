...
## Notifications
The RhoConnectClient system uses notifications to provide information about the sync process to a Rhodes application.  Notifications can be setup once for the duration of runtime or each time a sync is triggered.  One a sync is processing for a model, notifications are called with parameters containing sync process state.  Your application can use this information to display different wait pages, progress bars, etc. Below are graphics of two scenarios; one wherein the sync finished with no issue, and the other when the sync has errors that need to be handled:

### Sync Without Errors
<img src="https://s3.amazonaws.com/rhodocs/guide/synchronization/notification-flow-ok.png">

1. First, the user sets up notifications for the model(s) using [`RhoConnectClient.setNotification`](../api/RhoConnectClient#msetNotification) and the framework stores the notification setting.
2. The client app initiates a sync using [RhoConnectClient.doSync()](../api/RhoConnectClient#mdoSync).
3. The sync is performed on the back-end.
4. The sync_notfy method is called with a `@params["status"]` of "ok".

This is the common flow for synchronization when no errors occur.

### Sync With Errors
<img src="https://s3.amazonaws.com/rhodocs/guide/synchronization/notification-flow-errors.png">

1. First, the user sets up notifications for the model(s) using [`RhoConnectClient.setNotification`](../api/RhoConnectClient#msetNotification) and the framework stores the notification setting.
2. The client app initiates a sync using [RhoConnectClient.doSync()](../api/RhoConnectClient#mdoSync).
3. The sync is performed on the back-end.
4. The sync_notfy method is called with a `@params["status"]` of "error". In this case, the `@params["server_errors]` hash will contain a hash of &lt;model&gt; objects of RhoConnect adapter errors (if one exists) consisting of one or more of the following: **"login-error"**, **"query-error"**, **"create-error"**, **"update-error"**, **"delete-error"**, **"logoff-error"**. These can be accessed as follows: `@params["server-errors"]["<action>-error"]`.
5. If you would like to perform some custom actions upon receiving these notification errors, you must add code to do so in your `sync_notify` method in your controller.
6. Call the method that corresponds to the error you experienced. For example, if you get notification that a delete-error occurred, you'll want to call the [`RhoConnectClient.on_sync_delete_error()`](../api/RhoConnectClient#mon_sync_delete_error) method so you can decide whether or not you would like to use the `:retry` action to try again to delete the record.
7. Once you have decided which action to take that action will be performed on the record locally and be synced in the next synchronization attempt.
