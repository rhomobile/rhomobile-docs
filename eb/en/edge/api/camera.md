...
### choosePicture(<span class="text-info">HASH</span> propertyMap)Choose a picture from the album.
...
####Returns
Synchronous Return:<ul><li>ARRAY : Array of objects.<ul><li>cameraArray : <span class='text-info'>SELF_INSTANCE: Rho.Camera</span><p> </p></li></ul></li></ul>
...
### getCameraByType(<span class="text-info">STRING</span> cameraType)Returns the camera of requested type if that camera exist - else return nil.
...
####Returns
Synchronous Return:<ul><li>SELF_INSTANCE : Camera with requested type or nil if not requested type does not exist.</li></ul>
...
### saveImageToDeviceGallery(<span class="text-info">STRING</span> pathToImage)Save an image to the device gallery.
...
### setDefault(<span class="text-info">SELF_INSTANCE: Rho.Camera</span> defaultInstance)This method allows you to set the attributes of the default object instance by passing in an object of the same class.

####Parameters
<ul><li>defaultInstance : <span class='text-info'>SELF_INSTANCE: Rho.Camera</span><p>An instance object that is of the same class. </p></li><li>callback : <span class='text-info'>CallBackHandler</span></li></ul>
...
### takePicture(<span class="text-info">HASH</span> propertyMap)Start the camera application to take a picture. The user can capture the displayed image either by interacting with the resident camera app or pressing the trigger or enter key, depending on the device in use.
...
* Constant: Rho.Camera.OUTPUT_FORMAT_DATAURI 
	* String: dataUri
	* Image will be returned as a Data URI object, this is a base 64 encoding of the image and can be used to easily embed the image on the page or store the image in a database.Platforms: Android,WM
####Access
